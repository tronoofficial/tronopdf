/* TronoPDF - Scan to PDF v2 | Web Worker + Cancel + Progress */
(function(){
...
self.onmessage = function(e) {
  ...
  if (data.type === 'buildPDF') {
    ...
    PDFLib.PDFDocument.create().then(function(pdf) {
      ...
      pages.forEach(function(pageData, idx) {
        chain = chain.then(function() {
          var percent = 10 + ((idx + 1) / total) * 80;
          self.postMessage({
            type: 'progress',
            percent: percent,
            msg: 'Building page ' + (idx + 1) + ' of ' + total
          });
          
          return pdf.embedJpg(pageData).then(function(img) {
      ...
      self.postMessage({
        type: 'error',
        msg: 'Build failed: ' + (err.message || err)
      });
    });
  }
};
...
html+='.sc-busy .st{color:#7a7a85;font-size:15px;margin-bottom:10px;min-height:20px}';
...
html+='.sc-cancel{margin-top:16px;background:#f4f5fa;color:#333;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;border:none;cursor:pointer}';
html+='.sc-cancel:hover{background:#e6e8f5}';
...
html+='</div></div>';
html+='<div class="sc-busy" id="scBusy"><h2>Building PDF...</h2><p class="st" id="scStatus">Working...</p><div class="sc-bar"><div id="scBarFill"></div></div><div class="sc-pct" id="scPct">0%</div><button class="sc-cancel" id="scCancel" type="button">✕ Cancel</button></div>';
...
var cancelBtn=document.getElementById('scCancel');
...
worker.onmessage = function(e) {
  ...
  if (data.type === 'progress') {
    document.getElementById('scPct').textContent = Math.round(data.percent) + '%';
    document.getElementById('scBarFill').style.width = data.percent + '%';
    statusEl.textContent = data.msg || 'Processing...';
  } else if (data.type === 'complete') {
    document.getElementById('scPct').textContent = '100%';
    document.getElementById('scBarFill').style.width = '100%';
    statusEl.textContent = 'Complete!';
    
    setTimeout(function() {
      busy.style.display = 'none';
      done.style.display = 'block';
      document.getElementById('scDoneInfo').textContent = data.pageCount + ' page(s) • ' + (data.bytes.length / 1024).toFixed(1) + ' KB • No watermark';
      ...
      toast('✓ PDF ready! (' + data.pageCount + ' pages)');
    }, 300);
  } else if (data.type === 'error') {
    busy.style.display = 'none';
    work.style.display = 'block';
    goBtn.disabled = false;
    toast('Build failed: ' + data.msg, true);
  }
};
...
goBtn.onclick=function(){
  ...
  document.getElementById('scPct').textContent='0%';
  document.getElementById('scBarFill').style.width='0%';
  statusEl.textContent='Starting...';
  cancelRequested=false;
  goBtn.disabled=true;
  
  /* Extract base64 data from data URLs and send to worker */
  var pageData = pages.map(function(p) {
    /* Remove "data:image/jpeg;base64," prefix */
    return p.data.split(',')[1];
  });
