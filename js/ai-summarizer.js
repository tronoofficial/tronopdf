/* TronoPDF - AI Summarizer v1 | extractive in-browser AI, no API */
(function(){
...
html+='.sm-go{background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;font-size:17px;font-weight:800;padding:14px 34px;border-radius:12px;border:none;cursor:pointer;box-shadow:0 14px 34px rgba(124,58,237,.35)}';
...
html+='</div>';
root.innerHTML=html;
var inEl=document.getElementById('smIn'),outEl=document.getElementById('smOut'),statsEl=document.getElementById('smStats');
...
function words(s){return s.toLowerCase().replace(/[^a-z0-9\u0900-\u097F\s]/g,' ').split(/\s+/).filter(function(w){return w&&!STOP[w];});}
...
document.getElementById('smGo').onclick=function(){
 ...
 if(!text){toast('Please paste text or upload a file first',true);return;}
 var ratio=parseFloat(document.getElementById('smLen').value)||0.2;
 var r=summarize(text,ratio);
 if(!r.sum){toast('Could not summarize - text too short',true);return;}
 outEl.value=r.sum;
 statsEl.innerHTML='<span class="sm-stat">Original: '+r.stats.orig+' words</span><span class="sm-stat">Summary: '+r.stats.sum+' words</span><span class="sm-stat">Reduced: '+r.stats.red+'%</span>';
 toast('✓ Summary ready!');
};
document.getElementById('smFileBtn').onclick=function(){document.getElementById('smFile').click();};
document.getElementById('smFile').onchange=function(){
 var f=this.files[0];if(!f){return;}
 if(/\.txt$|text\/plain/.test(f.name)||f.type==='text/plain'){
  var rd=new FileReader();
  rd.onload=function(){inEl.value=rd.result;toast('✓ Text loaded!');};
  rd.readAsText(f);
 }else{
  loadJS(PDFJS_SRC,function(err){
   if(err||!window.pdfjsLib){toast('PDF engine failed',true);return;}
   window.pdfjsLib.GlobalWorkerOptions.workerSrc=PDFJS_WORKER;
   f.arrayBuffer().then(function(b){
    return window.pdfjsLib.getDocument({data:b}).promise.then(function(d){
     var chain=Promise.resolve();var all='';
     for(var i=1;i<=d.numPages;i++){
      (function(n){chain=chain.then(function(){return d.getPage(n).then(function(p){return p.getTextContent().then(function(tc){tc.items.forEach(function(it){all+=it.str+' ';});all+='\n';});});});})(i);
     }
     return chain.then(function(){inEl.value=all;toast('✓ PDF text loaded!');});
    });
   }).catch(function(){toast('Could not read PDF',true);});
  });
 }
 this.value='';
};
document.getElementById('smCopy').onclick=function(){
 if(!outEl.value){toast('No summary yet',true);return;}
 if(navigator.clipboard){navigator.clipboard.writeText(outEl.value).then(function(){toast('✓ Copied!');});}
...
document.getElementById('smDl').onclick=function(){
 ...
 var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='summary.txt';
