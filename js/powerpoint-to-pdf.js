/* TronoPDF - PowerPoint to PDF v2 | Cancel + Yield Points + Progress */
(function(){
...
html+='.pp-big:disabled{opacity:.5;cursor:not-allowed}';
...
html+='</div></div>';
html+='<div class="pp-busy" id="ppBusy"><h2>Converting to PDF...</h2><p class="st" id="ppStatus">Working...</p><div class="pp-bar"><div id="ppBarFill"></div></div><div class="pp-pct" id="ppPct">0%</div><button class="pp-cancel" id="ppCancel" type="button">✕ Cancel</button></div>';
...
goBtn.onclick=async function(){
  ...
  statusEl.textContent='Capturing slides...';
  ...
  try{
    ...
    for(var i=0;i<slides.length;i++){
      ...
      var percent=5+((i+1)/slides.length)*85;
      document.getElementById('ppPct').textContent=Math.round(percent)+'%';
      document.getElementById('ppBarFill').style.width=percent+'%';
      statusEl.textContent='Capturing slide '+(i+1)+' of '+slides.length+'...';
      ...
      if(i===0){
        pdf=new jsPDF({orientation:wPt>hPt?'landscape':'portrait',unit:'pt',format:[Math.max(wPt,hPt),Math.min(wPt,hPt)]});
      }else{
        pdf.addPage([Math.max(wPt,hPt),Math.min(wPt,hPt)],wPt>hPt?'landscape':'portrait');
      }
      
      var img=canvas.toDataURL('image/jpeg',0.92);
      pdf.addImage(img,'JPEG',0,0,wPt,hPt);
      
      /* Yield after each slide to keep UI responsive */
      await yieldToUI();
    }
    ...
    document.getElementById('ppPct').textContent='95%';
    document.getElementById('ppBarFill').style.width='95%';
    ...
    setTimeout(function(){
      ...
      document.getElementById('ppDoneInfo').textContent=slides.length+' slide(s) • '+(bytes.byteLength/1024).toFixed(1)+' KB';
      ...
      goBtn.disabled=false;
      toast('✓ PDF ready! ('+slides.length+' slides)');
    },300);
