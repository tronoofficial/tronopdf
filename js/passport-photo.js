/* TronoPDF - Passport Photo Maker v3 | presets, crop+zoom, bg replace, exact KB, print sheet */
(function(){
...
html+='.pp-big{background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;font-size:20px;font-weight:800;padding:20px 70px;border-radius:12px;border:none;cursor:pointer;box-shadow:0 14px 34px rgba(124,58,237,.35)}';
...
html+='.pp-zone.on{border-color:#7c3aed;background:#f3f0ff}';
html+='.pp-work{display:none;background:#f7f6fc;border-radius:14px;overflow:hidden}';
html+='.pp-main{display:flex;min-height:640px}';
html+='.pp-crop{flex:1;padding:40px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px}';
html+='.pp-frame{position:relative;border-radius:10px;overflow:hidden;box-shadow:0 10px 40px rgba(30,20,60,.18);cursor:move;touch-action:none;background:#e5e7eb}';
html+='.pp-frame canvas{display:block}';
html+='.pp-hint{font-size:13px;color:#9a9aa5}';
html+='.pp-zoom{display:flex;align-items:center;gap:12px;width:320px}';
html+='.pp-zoom input{flex:1;accent-color:#7c3aed}';
html+='.pp-side{width:400px;background:#fff;border-left:1px solid #eceaf6;padding:26px;display:flex;flex-direction:column;overflow-y:auto}';
html+='.pp-side h2{font-size:22px;font-weight:900;text-align:center;margin-bottom:6px}';
html+='.pp-sub{text-align:center;font-size:13px;color:#9a9aa5;margin-bottom:16px}';
html+='.pp-lbl{font-size:12px;font-weight:800;color:#4b4b5a;margin:12px 0 6px}';
html+='.pp-inp{width:100%;padding:11px 14px;border:1px solid #ddd;border-radius:10px;font-size:14px;font-family:inherit;background:#fff}';
html+='.pp-info{background:#ede9fe;border-radius:10px;padding:10px 14px;font-size:13px;color:#5b21b6;font-weight:700;text-align:center;margin-top:8px}';
html+='.pp-row{display:flex;gap:10px;align-items:center}';
html+='.pp-row input[type=number]{flex:1;padding:10px;border:1px solid #ddd;border-radius:10px;font-size:14px;min-width:0}';
html+='.pp-bg{display:flex;gap:10px}';
html+='.pp-bg button{flex:1;padding:10px;border:2px solid #eceaf6;border-radius:10px;font-size:12px;font-weight:800;cursor:pointer;background:#fff;color:#4b4b5a}';
html+='.pp-bg button.active{border-color:#7c3aed;color:#7c3aed;background:#f3f0ff}';
html+='.pp-row input[type=range]{flex:1;accent-color:#7c3aed}';
html+='.pp-chk{display:flex;gap:8px;align-items:center;margin:8px 0}';
html+='.pp-chk input{width:16px;height:16px;accent-color:#7c3aed}';
html+='.pp-chk label{font-size:13px;font-weight:600;cursor:pointer}';
html+='.pp-quick{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}';
html+='.pp-quick button{border:1px solid #eceaf6;background:#f7f6fc;color:#4b4b5a;font-size:11px;font-weight:800;padding:6px 12px;border-radius:999px;cursor:pointer}';
...
html+='.pp-dl{display:inline-block;background:#16a34a;color:#fff;font-weight:800;font-size:15px;padding:15px 34px;border-radius:12px;box-shadow:0 12px 28px rgba(22,163,74,.3)}';
html+='.pp-dl.sheet{background:#7c3aed;box-shadow:0 12px 28px rgba(124,58,237,.3)}';
html+='.pp-again{display:inline-block;background:#f4f5fa;color:#333;font-weight:700;font-size:14px;padding:14px 26px;border-radius:12px;border:none;cursor:pointer}';
html+='@media(max-width:900px){.pp-main{flex-direction:column}.pp-side{width:auto;border-left:none;border-top:1px solid #eceaf6}}';
html+='</style>';
html+='<div class="pp-wrap">';
html+='<div id="ppPick"><div class="pp-hero"><h1>Passport Photo Maker</h1><p>Perfect ID photos in seconds - correct size, clean background, print-ready sheet.</p>';
html+='<div class="pp-zone" id="ppZone"><button class="pp-big" id="ppBtn" type="button">Select Photo</button><p class="pp-drop-hint">or drop a photo here</p></div></div></div>';
html+='<div class="pp-work" id="ppWork"><div class="pp-main"><div class="pp-crop"><div class="pp-frame" id="ppFrame"><canvas id="ppCanvas"></canvas></div><div class="pp-zoom"><span style="font-size:12px;font-weight:800">🔍</span><input type="range" id="ppZoom" min="50" max="300" value="100"/></div><p class="pp-hint">Drag photo to position • Zoom to adjust • Face should fill the frame</p></div>';
html+='<aside class="pp-side"><h2>Photo settings</h2><p class="pp-sub">Everything set automatically for your document</p>';
html+='<div class="pp-lbl">Document type</div><select class="pp-inp" id="ppPreset"><option value="in_passport">India Passport / Visa (35×45 mm)</option><option value="in_pan">India PAN Card (25×35 mm)</option><option value="us">US Passport / Visa (2×2 inch)</option><option value="uk">UK Passport (35×45 mm)</option><option value="schengen">Schengen Visa (35×45 mm)</option><option value="canada">Canada Passport (50×70 mm)</option><option value="china">China Passport (33×48 mm)</option><option value="custom">Custom size (mm)</option></select>';
html+='<div class="pp-row" id="ppCustomRow" style="display:none;margin-top:8px"><input type="number" id="ppMmW" value="35" min="10" max="100"/><span style="font-size:12px;color:#9a9aa5">×</span><input type="number" id="ppMmH" value="45" min="10" max="100"/><span style="font-size:12px;color:#9a9aa5">mm</span></div>';
html+='<div class="pp-info" id="ppInfo"></div>';
html+='<div class="pp-lbl">Background</div><div class="pp-bg"><button class="active" data-bg="orig" type="button">Original</button><button data-bg="white" type="button">White</button><button data-bg="blue" type="button">Blue</button><button data-bg="red" type="button">Red</button></div>';
html+='<div class="pp-lbl">Background match: <span id="ppTolVal">45</span></div><div class="pp-row"><input type="range" id="ppTol" min="5" max="100" value="45"/></div>';
html+='<div class="pp-chk"><input type="checkbox" id="ppKb"/><label for="ppKb">Set exact file size (for forms)</label></div>';
html+='<div id="ppKbBox" style="display:none"><div class="pp-row"><input type="number" id="ppKbVal" value="50" min="5"/><span style="font-size:12px;color:#9a9aa5">KB</span></div><div class="pp-quick"><button type="button" data-kb="20">20 KB</button><button type="button" data-kb="50">50 KB</button><button type="button" data-kb="100">100 KB</button></div></div>';
...
var bgBtns=document.querySelectorAll('.pp-bg button');
for(var bi=0;bi<bgBtns.length;bi++){
 bgBtns[bi].onclick=function(){
