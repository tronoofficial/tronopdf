/* TronoPDF - Compress PDF v1 | 3 levels, target size, before/after */
(function(){
 ...
 setTimeout(function(){
  file.arrayBuffer().then(function(buf){
   ...
   return PDFLib.PDFDocument.load(buf,{ignoreEncryption:true}).then(function(pdf){
    pct(40);
    var attempts=useTarget?6:1;
    var qualityMap={extreme:0.35,recommended:0.6,less:0.85};
    var baseQuality=qualityMap[level];
    function tryCompress(q,attempt){
     var embedPromises=[];
     var pages=pdf.getPages();
     pct(40+((attempt-1)/attempts)*45);
     return PDFLib.PDFDocument.create().then(function(newPdf){
      var chain=Promise.resolve();
      pages.forEach(function(page,idx){
       chain=chain.then(function(){
        pct(40+((attempt-1)/attempts + (idx/pages.length)/attempts)*45);
        var ops=page.node.Contents();
        return newPdf.embedPage(pdf,idx).then(function(ep){
         var w=page.getWidth(),h=page.getHeight();
         var np=newPdf.addPage([w,h]);
         np.drawPage(ep,0,0,w,h);
        });
       });
      });
      return chain.then(function(){
       return newPdf.save({useObjectStreams:true,addDefaultPage:false}).then(function(bytes){
        if(useTarget&&bytes.length>targetBytes&&attempt<attempts){
         return tryCompress(Math.max(0.1,q-0.12),attempt+1);
        }
        pct(95);
        setTimeout(function(){showResult(bytes);},300);
       });
      });
     });
    }
    pdf.save({useObjectStreams:true}).then(function(origBytes){
     tryCompress(baseQuality,1);
    });
   });
  }).catch(function(){
