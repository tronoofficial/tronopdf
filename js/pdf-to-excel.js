/* TronoPDF - PDF to Excel v1 | pdf.js position extraction + SheetJS write, browser-only */
(function(){
...
html+='.pe-busy .st{color:#7a7a85;font-size:15px;margin-bottom:26px}';
...
html+='</div></div>';
html+='<div class="pe-busy" id="peBusy"><h2>Extracting tables...</h2><p class="st" id="peStatus">Working...</p><div class="pe-bar"><div id="peBarFill"></div></div><div class="pe-pct" id="pePct">0%</div></div>';
...
html+='</div>';
root.innerHTML=html;
var file=null,doc=null,allPages=[];
...
function pct(p){document.getElementById('pePct').textContent=Math.round(p)+'%';document.getElementById('peBarFill').style.width=p+'%';}
function setStatus(s){document.getElementById('peStatus').textContent=s;}
...
function loadFile(f){
 ...
 loadJS(PDFJS_SRC).then(function(){
  ...
  doc=d;
  return extractPage(1).then(function(aoa){
   allPages=[aoa];
   pick.style.display='none';done.style.display='none';work.style.display='block';
   renderPreview(aoa);
   toast('✓ PDF loaded ('+d.numPages+' pages)');
  });
 }).catch(function(e){pick.style.display='block';toast('Could not read PDF',true);});
}
...
document.getElementById('peGo').onclick=async function(){
 ...
 try{
  ...
  if(layout==='single'){
   ...
   for(var i=1;i<=totalPages;i++){
    ...
    var aoa=await extractPage(i);
    combined=combined.concat(aoa,[[]]);
    pct(5+(i/totalPages)*80);
   }
   var ws=XLSX.utils.aoa_to_sheet(combined);
   XLSX.utils.book_append_sheet(wb,ws,'Data');
  }else{
   for(var j=1;j<=totalPages;j++){
    setStatus('Extracting page '+j+' of '+totalPages+'...');
    var a2=await extractPage(j);
    if(a2.length===0){a2=[['(no text)']];}
    var ws2=XLSX.utils.aoa_to_sheet(a2);
    XLSX.utils.book_append_sheet(wb,ws2,'Page '+j);
    pct(5+(j/totalPages)*80);
   }
  }
  ...
  setTimeout(function(){
   ...
   document.getElementById('peDoneInfo').textContent=totalPages+' page(s) • '+(blob.size/1024).toFixed(1)+' KB';
