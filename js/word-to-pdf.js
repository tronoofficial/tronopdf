/* TronoPDF - Word to PDF v2 | mammoth Worker + Cancel + Yield Points */
(function(){
...
/* Web Worker for mammoth parsing (CPU intensive for large docs) */
var workerCode = `
...
self.onmessage = function(e) {
  ...
  if (data.type === 'parse') {
    self.postMessage({type: 'progress', percent: 15, msg: 'Parsing Word document...'});
    
    mammoth.convertToHtml({arrayBuffer: data.buffer}).then(function(res) {
      self.postMessage({
        type: 'complete',
        html: res.value
      });
      ...
      self.postMessage({
        type: 'error',
        msg: 'Parse failed: ' + (err.message || err)
      });
    });
  }
};
...
html+='.wp-big:disabled{opacity:.5;cursor:not-allowed}';
...
html+='.wp-busy .st{color:#7a7a85;font-size:15px;margin-bottom:10px;min-height:20px}';
...
html+='.wp-cancel{margin-top:16px;background:#f4f5fa;color:#333;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;border:none;cursor:pointer}';
html+='.wp-cancel:hover{background:#e6e8f5}';
...
html+='<div class="wp-busy" id="wpBusy"><h2>Converting to PDF...</h2><p class="st" id="wpStatus">Working...</p><div class="wp-bar"><div id="wpBarFill"></div></div><div class="wp-pct" id="wpPct">0%</div><button class="wp-cancel" id="wpCancel" type="button">✕ Cancel</button></div>';
...
var cancelBtn=document.getElementById('wpCancel');
...
worker.onmessage = function(e) {
  ...
  if (data.type === 'progress') {
    document.getElementById('wpPct').textContent = Math.round(data.percent) + '%';
    document.getElementById('wpBarFill').style.width = data.percent + '%';
    ...
    /* Continue with html2canvas on main thread */
    renderWithHtml2Canvas(data.html);
...
/* Yield helper - gives UI thread a chance to breathe */
function yieldToUI() {
...
async function renderWithHtml2Canvas(htmlContent) {
  if (cancelRequested) {
    busy.style.display = 'none';
    pick.style.display = 'block';
    btn.disabled = false;
  ...
  document.getElementById('wpPct').textContent = '40%';
  document.getElementById('wpBarFill').style.width = '40%';
  ...
  holder.innerHTML = '<style>h1{font-size:26px}h2{font-size:21px}h3{font-size:18px}p{margin:0 0 12px}img{max-width:100%;height:auto}table{border-collapse:collapse}td,th{border:1px solid #999;padding:6px}</style>' + htmlContent;
  document.body.appendChild(holder);
  
  try {
    /* html2canvas must run on main thread (DOM dependency) */
    ...
    document.getElementById('wpPct').textContent = '75%';
    document.getElementById('wpBarFill').style.width = '75%';
    statusEl.textContent = 'Building PDF...';
    
    /* Yield before heavy PDF building */
    await yieldToUI();
    ...
    for (var i = 0; i < pages; i++) {
      ...
      var pct = 75 + ((i + 1) / pages) * 20;
      document.getElementById('wpPct').textContent = Math.round(pct) + '%';
      document.getElementById('wpBarFill').style.width = pct + '%';
      statusEl.textContent = 'Building page ' + (i + 1) + ' of ' + pages;
      
      /* Yield every 2 pages to keep UI responsive */
      if (i % 2 === 1) {
        await yieldToUI();
      }
    }
    ...
    document.getElementById('wpPct').textContent = '100%';
    document.getElementById('wpBarFill').style.width = '100%';
    ...
    setTimeout(function() {
      ...
      document.getElementById('wpDoneInfo').textContent = pages + ' page(s) • ' + (bytes.byteLength / 1024).toFixed(1) + ' KB';
      
      var dl = document.getElementById('wpDl');
      dl.href = URL.createObjectURL(blob);
      dl.download = currentFileName.replace(/\.docx?$/i, '') + '.pdf';
      
      btn.disabled = false;
      toast('✓ PDF ready! (' + pages + ' pages)');
    }, 300);
    ...
    pick.style.display = 'block';
    btn.disabled = false;
    toast('Rendering failed: ' + (err.message || err), true);
  }
}
...
inp.onchange = function() {
  if (inp.files[0]) { convert(inp.files[0]); }
  inp.value = '';
};
...
function convert(f) {
  if (!/\.docx?$/.test(f.name) && f.type.indexOf('word') < 0) {
    toast('Please select a .docx Word file', true);
    return;
  }
  ...
  document.getElementById('wpPct').textContent = '0%';
  document.getElementById('wpBarFill').style.width = '0%';
  statusEl.textContent = 'Loading converters...';
  cancelRequested = false;
  btn.disabled = true;
  
  /* Load libraries first */
  Promise.all([loadJS(H2C_SRC), loadJS(JSPDF_SRC)]).then(function() {
    if (cancelRequested) {
      busy.style.display = 'none';
      pick.style.display = 'block';
      btn.disabled = false;
    ...
    document.getElementById('wpPct').textContent = '5%';
    document.getElementById('wpBarFill').style.width = '5%';
    ...
    /* Send to worker for mammoth parsing */
    worker.postMessage({
      type: 'parse',
      buffer: buf
    }, [buf]);
