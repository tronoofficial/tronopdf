/* TronoPDF - Image Converter v1 | JPG PNG WEBP BMP, batch, quality, resize, ZIP */
(function(){
...
html+='.ic-inp{width:100%;padding:11px 14px;border:1px solid #ddd;border-radius:10px;font-size:14px;font-family:inherit;background:#fff}';
...
html+='<div id="icPick"><div class="ic-hero"><h1>Image Converter</h1><p>Convert JPG, PNG, WEBP and BMP - batch, fast and 100% private.</p>';
...
html+='</div>';
html+='<div class="ic-lbl">Quality: <span id="icQVal">90</span>%</div><div class="ic-row"><input type="range" id="icQ" min="10" max="100" value="90"/></div>';
...
var elResize=document.getElementById('icResize'),elResizeBox=document.getElementById('icResizeBox');
...
function render(){
 ...
 files.forEach(function(it){
  ...
  if(it.error){
   ...
   body+='<div class="ic-badge">'+it.fmt.toUpperCase()+'</div><div class="ic-sizes"><span class="ic-old">'+fmtB(it.f.size)+'</span><span>→</span><span class="ic-new">'+fmtB(it.result.bytes)+'</span></div><a class="ic-dl" href="'+it.result.dataURL+'" download="converted.'+it.fmt+'">⬇ Download</a>';
  }else{
...
function convertOne(it,fmt,q,resize,w,h,cb){
 var img=new Image();
 img.onload=function(){
  var tw=w||img.width,th=h||img.height;
  if(resize&&w&&!h){th=Math.round(img.height*w/img.width);}
  if(resize&&!w&&h){tw=Math.round(img.width*h/img.height);}
  var c=document.createElement('canvas');c.width=tw;c.height=th;
  var x=c.getContext('2d');
  x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';
  if(fmt==='jpg'||fmt==='bmp'){x.fillStyle='#fff';x.fillRect(0,0,tw,th);}
  x.drawImage(img,0,0,tw,th);
  var mime,d;
  if(fmt==='jpg'){mime='image/jpeg';d=c.toDataURL(mime,q/100);}
  else if(fmt==='png'){mime='image/png';d=c.toDataURL(mime);}
  else if(fmt==='webp'){mime='image/webp';d=c.toDataURL(mime,q/100);}
  else if(fmt==='bmp'){
   try{d=canvasToBmp(c);}catch(err){d=c.toDataURL('image/jpeg',q/100);}
  }else{mime='image/jpeg';d=c.toDataURL(mime,q/100);}
  cb({dataURL:d,bytes:atob(d.split(',')[1]).length,fmt:fmt});
 };
 img.onerror=function(){cb(null);};
 img.src=it.url;
}
// Simple BMP encoder for browsers that don't support canvas.toBlob('image/bmp')
function canvasToBmp(c){
 var ctx=c.getContext('2d');var w=c.width,h=c.height;
 var imgD=ctx.getImageData(0,0,w,h);var p=imgD.data;
...
document.getElementById('icGo').onclick=function(){
 ...
 var doneCount=0;
 files.forEach(function(it){
  convertOne(it,outFmt,q,resize,w,h,function(res){
   ...
   if(doneCount===files.length){
    ...
    if(files.length>1&&window.JSZip){
     ...
     files.forEach(function(f2,idx){
      if(f2.result){
       zip.file('converted-'+(idx+1)+'.'+f2.result.fmt,f2.result.dataURL.split(',')[1],{base64:true});
      }
     });
     zip.generateAsync({type:'blob'}).then(function(blob){
      var a=document.createElement('a');
      a.href=URL.createObjectURL(blob);a.download='tronopdf-converted.zip';
      a.textContent='⬇ Download all as ZIP';
      a.style.cssText='display:block;background:#16a34a;color:#fff;font-weight:800;font-size:15px;padding:14px;border-radius:12px;text-align:center;margin-top:14px;box-shadow:0 12px 28px rgba(22,163,74,.3)';
