/* TronoPDF - PDF Forms v2 | Web Worker + Cancel + Progress */
(function(){
...
/* Web Worker for PDF form processing */
var workerCode = `
...
self.onmessage = function(e) {
  ...
  if (data.type === 'generate') {
    ...
    PDFLib.PDFDocument.load(buffer, {ignoreEncryption: true}).then(function(pdf) {
      ...
      self.postMessage({type: 'progress', percent: 25, msg: 'Preparing form...'});
      ...
      for (var n = 1; n <= totalPages; n++) {
        var percent = 25 + ((n - 1) / totalPages) * 65;
        self.postMessage({
          type: 'progress',
          percent: percent,
          msg: 'Processing page ' + n + ' of ' + totalPages
        });
        ...
        list.forEach(function(f, idx) {
          var pdfY = ph - f.y - f.h;
          var name = f.name + '_' + n + '_' + idx;
          fieldCount++;
          
          try {
            if (mode === 'create') {
              if (f.type === 'text') {
                var t = form.createTextField(name);
                t.setText(f.value || '');
                t.addToPage(pg, {x: f.x, y: pdfY, width: f.w, height: f.h});
              } else if (f.type === 'checkbox') {
                var c = form.createCheckBox(name);
                if (f.value) c.check();
                c.addToPage(pg, {x: f.x, y: pdfY, width: f.w, height: f.h});
              } else if (f.type === 'radio') {
                var g = form.createRadioGroup(name);
                g.addOptionToPage('opt', {page: pg, x: f.x, y: pdfY, width: f.w, height: f.h});
              } else if (f.type === 'dropdown') {
                var d = form.createDropdown(name);
                (f.options || []).forEach(function(o) { d.addOptions(o); });
                if (f.value) d.select(f.value);
                d.addToPage(pg, {x: f.x, y: pdfY, width: f.w, height: f.h});
              } else if (f.type === 'date') {
                var dt = form.createTextField(name);
                dt.setText(f.value || 'YYYY-MM-DD');
                dt.addToPage(pg, {x: f.x, y: pdfY, width: f.w, height: f.h});
              } else if (f.type === 'signature') {
                var s = form.createTextField(name);
                s.setText('');
                s.addToPage(pg, {x: f.x, y: pdfY, width: f.w, height: f.h});
              }
            } else if (form) {
              ...
              if (existing) {
                try {
                  if (f.type === 'checkbox') {
                    if (f.value) existing.check();
                    else existing.uncheck();
                  } else if (f.type === 'radio') {
                    existing.select(f.value || '');
                  } else if (f.type === 'dropdown') {
                    existing.select(f.value || '');
                  } else {
                    existing.setText(f.value || '');
                  }
                } catch(e) {}
              }
            }
      ...
      self.postMessage({
        type: 'error',
        msg: 'Form processing failed: ' + (err.message || err)
      });
    });
  }
};
...
html+='.fm-go:disabled{opacity:.5;cursor:not-allowed}';
...
html+='.fm-busy .st{color:#7a7a85;font-size:15px;margin-bottom:10px;min-height:20px}';
html+='.fm-busy .st2{color:#7c3aed;font-size:13px;font-weight:700;margin-bottom:16px;min-height:18px}';
...
html+='.fm-pct{font-size:36px;font-weight:900}';
html+='.fm-cancel{margin-top:16px;background:#f4f5fa;color:#333;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;border:none;cursor:pointer}';
html+='.fm-cancel:hover{background:#e6e8f5}';
...
html+='<div class="fm-busy" id="fmBusy"><h2>Processing...</h2><p class="st" id="fmStatus">Working...</p><p class="st2" id="fmStatus2"></p><div class="fm-bar"><div id="fmBarFill"></div></div><div class="fm-pct" id="fmPct">0%</div><button class="fm-cancel" id="fmCancel" type="button">✕ Cancel</button></div>';
...
var cancelBtn=document.getElementById('fmCancel');
var statusEl=document.getElementById('fmStatus');
var status2El=document.getElementById('fmStatus2');
...
worker.onmessage = function(e) {
  ...
  if (data.type === 'progress') {
    document.getElementById('fmPct').textContent = Math.round(data.percent) + '%';
    document.getElementById('fmBarFill').style.width = data.percent + '%';
    statusEl.textContent = data.msg || 'Processing...';
  } else if (data.type === 'complete') {
    document.getElementById('fmPct').textContent = '100%';
    document.getElementById('fmBarFill').style.width = '100%';
    ...
    setTimeout(function() {
      ...
      document.getElementById('fmDoneInfo').textContent = (mode==='create'?'Created ':'Filled ') + data.fieldCount + ' field(s) • ' + fmtB(data.bytes.length);
      ...
      document.getElementById('fmGo').disabled = false;
      toast('✓ PDF ready!');
    }, 300);
...
function syncUI(){
  ...
  list.forEach(function(f){
    ...
    d.className='fm-field'+(selField===f?' sel':'');
    d.style.left=(f.x*scale)+'px';
    d.style.top=(f.y*scale)+'px';
    d.style.width=(f.w*scale)+'px';
    d.style.height=(f.h*scale)+'px';
    ...
    if(f.type==='text'){d.innerHTML='<input type="text" placeholder="Text field" value="'+(f.value||'')+'"/>';}
      ...
      var sel='<select>';
      (f.options||[]).forEach(function(o){sel+='<option '+(o===f.value?'selected':'')+'>'+o+'</option>';});
      sel+='</select>';
      d.innerHTML=sel;
    }
    else if(f.type==='date'){d.innerHTML='<input type="date" value="'+(f.value||'')+'"/>';}
    else if(f.type==='signature'){
      d.style.border='2px dashed #7c3aed';
      d.style.background='rgba(124,58,237,.05)';
      d.textContent='✍️ Signature';
    }
    ...
    if(inp){
      inp.oninput=inp.onchange=function(){
        f.value=inp.type==='checkbox'||inp.type==='radio'?inp.checked:inp.value;
      };
    }
    ...
    d.addEventListener('pointerdown',function(e){
      ...
      function mv(ev){
        if(!on)return;
        f.x+=(ev.clientX-lx)/scale;
        f.y+=(ev.clientY-ly)/scale;
        lx=ev.clientX;
        ly=ev.clientY;
        d.style.left=(f.x*scale)+'px';
        d.style.top=(f.y*scale)+'px';
    ...
    li.className='fm-li'+(selField===f?' sel':'');
...
function detectExistingFields(){
            ...
            return p.getAnnotations().then(function(annots){
              ...
              annots.forEach(function(a){
                if(a.subtype==='Widget'&&a.fieldType){
                  ...
                  fields[n].push({
                    type:type,x:rect[0],y:pageH-rect[3],
                    w:rect[2]-rect[0],h:rect[3]-rect[1],
                    name:a.fieldName||'f',value:a.fieldValue||''
                  });
                }
              });
            });
  ...
  chain.then(function(){
    syncUI();
    var total=0;
    Object.keys(fields).forEach(function(k){total+=fields[k].length;});
    if(total>0)toast('✓ Detected '+total+' existing fields');
  });
}
...
function generate(){
  ...
  document.getElementById('fmBarFill').style.width='0%';
  statusEl.textContent='Starting...';
  status2El.textContent='';
  cancelRequested=false;
  document.getElementById('fmGo').disabled=true;
