...
/* ===== RENDER RELATED TOOLS (with workflow hints) ===== */
function renderRelatedTools(){
  ...
  var workflows={
    'compress-pdf':{title:'Common Workflow',desc:'Merge PDFs → Compress → Add page numbers → Protect'},
    'merge-pdf':{title:'Next Steps',desc:'After merging: Compress to reduce size, then protect with password'},
    'split-pdf':{title:'What\'s Next?',desc:'Split pages → Compress individually → Merge selected pages'},
    'jpg-to-pdf':{title:'Complete Workflow',desc:'Convert images → Compress PDF → Add watermark → Protect'},
    'pdf-to-word':{title:'Edit & Share',desc:'Convert to Word → Edit content → Convert back to PDF → Sign'},
    'sign-pdf':{title:'Security Steps',desc:'Sign document → Add watermark → Password protect → Share'},
    'protect-pdf':{title:'Full Security',desc:'Protect with password → Add watermark → Redact sensitive info'},
    'ocr-pdf':{title:'After OCR',desc:'Extract text → Translate → Summarize → Convert to Word'}
  };

  var workflow=workflows[current];

  var html='<div class="rt-section">';
  html+='<h2 class="rt-title">🔗 Related Tools You Might Need</h2>';
  html+='<p class="rt-subtitle">Complete your document workflow with these tools</p>';
  
  if(workflow){
    html+='<div class="rt-workflow">';
    html+='<div class="rt-wf-title">'+workflow.title+'</div>';
    html+='<div class="rt-wf-desc">'+workflow.desc+'</div>';
    html+='</div>';
  }
  ...
  style.textContent='.rt-section{max-width:1200px;margin:40px auto;padding:0 20px}.rt-title{font-size:26px;font-weight:900;color:#1e1e2e;margin-bottom:6px}.rt-subtitle{font-size:14px;color:#8a8a99;margin-bottom:20px}.rt-workflow{background:linear-gradient(135deg,#f3f0ff,#ede9fe);border:1px solid #e9d5ff;border-radius:14px;padding:20px;margin-bottom:24px}.rt-wf-title{font-size:16px;font-weight:800;color:#7c3aed;margin-bottom:8px}.rt-wf-desc{font-size:14px;color:#4b5563;line-height:1.6}.rt-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px}.rt-card{display:flex;align-items:center;gap:14px;padding:16px 18px;background:#fff;border:1px solid #eceaf6;border-radius:14px;text-decoration:none;color:inherit;transition:all .25s ease}.rt-card:hover{border-color:#7c3aed;box-shadow:0 8px 24px rgba(124,58,237,.12);transform:translateY(-3px)}.rt-icon{width:48px;height:48px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0}.rt-info{flex:1;min-width:0}.rt-name{font-size:14px;font-weight:800;color:#1e1e2e;margin-bottom:2px}.rt-desc{font-size:12px;color:#8a8a99;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rt-arrow{font-size:16px;color:#c4c4d4;transition:transform .2s}.rt-card:hover .rt-arrow{transform:translateX(4px);color:#7c3aed}html[data-theme="dark"] .rt-workflow{background:linear-gradient(135deg,#1e1e30,#232336);border-color:#32324a}html[data-theme="dark"] .rt-wf-title{color:#c9b8ff}html[data-theme="dark"] .rt-wf-desc{color:#b9b9cc}@media(max-width:600px){.rt-grid{grid-template-columns:1fr}.rt-section{padding:0 16px;margin:30px auto}}';
...
(function(){
  ...
  function apply(){
    ...
    setMeta('og:description','Fast, private PDF tools that work in your browser. No signup, no uploads.', false);
    ...
    setMeta('twitter:description','Fast, private PDF tools. Rule Your PDFs.',true);
...
(function(){
  ...
  if(!document.querySelector('meta[name="description"]')){
    ...
    md.content=DESC[file]||'TronoPDF - Fast, private online PDF tools. Merge, split, compress, convert, sign & edit PDFs. No signup required.';
...
/* ===== TASK 18 v2: LANGUAGE SELECTOR (LAZY LOAD) ===== */
(function(){
  ...
  '.lang-btn.loading{opacity:.6;cursor:wait}'+
  ...
  function loadGoogleTranslate(cb){
    if(loaded){cb();return;}
    if(loading){var t=setInterval(function(){if(loaded){clearInterval(t);cb();}},100);return;}
    loading=true;
    ...
    g.onerror=function(){loading=false;if(btn)btn.classList.remove('loading');};
    document.head.appendChild(g);
  }

  function waitSelect(cb){
    var t=0;
    (function w(){
      var sel=document.querySelector('.goog-te-combo');
      if(sel){ if(pending){sel.value=pending;sel.dispatchEvent(new Event('change'));} if(cb)cb(); return; }
      if(t>50){if(cb)cb();return;} t++; setTimeout(w,200);
    })();
  }

  function applyCode(code,cb){
    var sel=document.querySelector('.goog-te-combo');
    if(sel){ sel.value=code; sel.dispatchEvent(new Event('change')); if(cb)cb(); }
    else {pending=code; if(cb)cb();}
  }
  ...
  function buildUI(){
    btn=document.createElement('button'); btn.type='button'; btn.className='lang-btn';
    var cur='English';
    try{ cur=localStorage.getItem('tronopdf-lang-name')||'English'; }catch(e){}
    btn.innerHTML='🌐 <span>'+cur+'</span> ▾';
    panel=document.createElement('div'); panel.className='lang-panel';
    var h='';
    ...
    /* FIRST INTERACTION: load Google Translate lazily */
    var firstClick=true;
    btn.addEventListener('click',function(e){
      ...
      if(firstClick){
        ...
        loadGoogleTranslate(function(){
          ...
          var savedLang='English';
          try{savedLang=localStorage.getItem('tronopdf-lang-name')||'English';}catch(x){}
          btn.querySelector('span').textContent=savedLang;
...
/* ===== TASK 34: FOOTER SOCIAL ICONS ===== */
(function(){
  var st=document.createElement('style');
  st.textContent='.pf-social{max-width:1200px;margin:0 auto 30px;padding:24px;text-align:center;border-top:1px solid rgba(255,255,255,.06)}'+
  '.pf-social-label{display:block;font-size:11px;text-transform:uppercase;letter-spacing:1.5px;color:#6a6a80;margin-bottom:14px;font-weight:700}'+
  '.pf-social-icons{display:flex;justify-content:center;gap:10px;flex-wrap:wrap}'+
  '.pf-social-icons a{width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:#c9c9dd;text-decoration:none;transition:all .2s}'+
  '.pf-social-icons a:hover{background:#7c3aed;border-color:#7c3aed;color:#fff;transform:translateY(-2px)}'+
  '.pf-social-icons a svg{width:18px;height:18px;fill:currentColor}'+
  '@media(max-width:560px){.pf-social-icons a{width:36px;height:36px}.pf-social-icons a svg{width:16px;height:16px}}';
  document.head.appendChild(st);

  function build(){
    var footer=document.querySelector('.pf-bottom');
    if(!footer)return;
    var section=document.createElement('div');
    section.className='pf-social';
    section.innerHTML='<span class="pf-social-label">Follow TronoPDF</span>'+
    '<div class="pf-social-icons">'+
      '<a href="https://x.com/tronopdf" target="_blank" rel="noopener" aria-label="X (Twitter)"><svg viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>'+
      '<a href="https://www.instagram.com/tronopdf" target="_blank" rel="noopener" aria-label="Instagram"><svg viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1112.324 0 6.162 6.162 0 01-12.324 0zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg></a>'+
      '<a href="https://www.facebook.com/profile.php?id=61593992880022" target="_blank" rel="noopener" aria-label="Facebook"><svg viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>'+
      '<a href="https://www.reddit.com/user/tronoofficial" target="_blank" rel="noopener" aria-label="Reddit"><svg viewBox="0 0 24 24"><path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 0 1 .042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 0 1 4.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 0 1 .14-.197.35.35 0 0 1 .238-.042l2.906.617a1.214 1.214 0 0 1 1.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 0 0-.231.094.33.33 0 0 0 0 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 0 0 .029-.463.33.33 0 0 0-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 0 0-.232-.095z"/></svg></a>'+
      '<a href="https://www.quora.com/profile/TronoPDF" target="_blank" rel="noopener" aria-label="Quora"><svg viewBox="0 0 24 24"><path d="M12.001.004C5.384.004.023 5.365.023 11.981c0 6.618 5.361 11.979 11.978 11.979 2.583 0 4.979-.822 6.934-2.218l-.017.016 2.782 1.803-1.066-2.887c1.66-1.937 2.67-4.455 2.67-7.21 0-6.617-5.361-11.98-11.978-11.98zM7.533 13.322c-.479-.959-1.036-1.775-1.669-2.449-.634-.675-1.196-1.067-1.686-1.177.197-.45.48-.856.852-1.22.414-.406.868-.646 1.363-.722-.025.191-.037.389-.037.593 0 .567.078 1.096.235 1.586.195.61.484 1.171.868 1.684-.002-.003-.007-.004-.011-.007.011.225.017.449.017.672 0 .487-.04.953-.12 1.397.051-.121.098-.243.141-.366.074-.214.047-.257.047-.011zm7.723-3.602c-.232-.557-.519-1.07-.86-1.538a5.804 5.804 0 00-1.065-1.102c-.41-.323-.818-.547-1.225-.673.442-.12.905-.181 1.388-.181.526 0 1.024.07 1.494.21.532.158.993.394 1.383.708.428.345.763.769 1.005 1.27.27.557.405 1.167.405 1.828 0 .667-.137 1.283-.41 1.849-.243.505-.58.93-1.01 1.277-.39.316-.85.554-1.383.714-.47.14-.968.21-1.494.21-.486 0-.95-.062-1.392-.184.41-.128.818-.353 1.228-.677.398-.315.753-.685 1.062-1.107.34-.468.625-.98.856-1.537.207-.495.31-1.02.31-1.577 0-.552-.102-1.074-.308-1.568zm-5.494 2.095c0-.667.138-1.284.414-1.85.245-.505.583-.93 1.015-1.276.393-.316.855-.554 1.388-.713.472-.14.973-.21 1.502-.21.532 0 1.035.07 1.508.21.536.16.998.398 1.388.714.43.346.765.772 1.005 1.276.27.567.404 1.183.404 1.85 0 .662-.135 1.274-.404 1.836-.24.502-.575.925-1.005 1.27-.39.313-.852.549-1.388.707-.473.14-.976.21-1.508.21-.53 0-1.03-.07-1.502-.21-.533-.158-.995-.394-1.388-.707-.432-.345-.77-.768-1.015-1.27-.276-.562-.414-1.174-.414-1.836z"/></svg></a>'+
      '<a href="https://www.linkedin.com/in/sahina-khatoon731/" target="_blank" rel="noopener" aria-label="LinkedIn"><svg viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.063 2.063 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>'+
    '</div>';
